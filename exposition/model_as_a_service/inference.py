...
MODEL_REGISTRY_FOLDER = "/home/jovyan/work/Formation-MLOps-2/models"
MODEL_PATH = os.path.join(MODEL_REGISTRY_FOLDER, '20200616-140746.joblib') 
DATA = pd.DataFrame(
    {"Wind_turbine_name": "R80721", "Date_time": "2017-02-08T08:00:00+01:00", "Ba_min": 44.99,
...
@app.route('/predict')
def predict_endpoint(ws1_avg=20):
    received_wind_speed_avg = request.args.get(ws1_avg)
    DATA["Ws1_avg"] = received_wind_speed_avg
    features = prepare_features(DATA, training_mode=False)
    prediction = predict(features=features, model_path=MODEL_PATH)['predictions']
